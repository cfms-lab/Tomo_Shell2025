...
//**********************************************************************************************
	...
	//Note: blockDim  = ( nWorksPerBlocks, 1, 1) x (maxD,	maxD, CU_TRI_PER_WORK)
		...
		{ 
			const CU_ULInt	pxl_ID		= slot_ID * CU_SLOT_CAPACITY_16 + *memNPxl;//pxl ID inside the target slot. 순서주의: cu_Add(	 memNPxl, 1) 앞에 와야 한다.
			...
			cu_Add(memNPxl, 1);//increase target slot's current # of pxls
			cu_Exch(cu_pZcrd + pxl_ID, new_z);
			cu_Exch(cu_pZnrm + pxl_ID, new_nZ);
#ifdef _CUDA_USE_SPLIT_AL_BE_IN_VOXELIZE_STEP
			cu_Exch(cu_pType + pxl_ID, (new_nZ > 0) ? cu_typeAl : cu_typeBe);// = splitAlBe()
#endif

			memNPxl = cu_nPixel + slot_ID;
			if (bShellMesh && *memNPxl < CU_SLOT_CAPACITY_16 - 1) //입력 데이터가 닫히지 않은 shell 형태인경우
			{
				cu_Add(memNPxl, 1);//increase target slot's current # of pxls
				const CU_ULInt	sub_ID = slot_ID * CU_SLOT_CAPACITY_16 + *memNPxl;
				cu_Exch(cu_pZcrd + sub_ID, (new_nZ > 0) ? new_z - 1 : new_z + 1);
				cu_Exch(cu_pZnrm + sub_ID, new_nZ * -1);
#ifdef _CUDA_USE_SPLIT_AL_BE_IN_VOXELIZE_STEP
				cu_Exch(cu_pType + sub_ID, (new_nZ > 0) ? cu_typeBe : cu_typeAl);// = splitAlBe()
#endif
			}
		}
